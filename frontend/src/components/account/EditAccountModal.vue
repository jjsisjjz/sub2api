  ...
  >
    ...
    >
      ...
      >
        <div class="flex items-center justify-between gap-4">
          <div>
            <label class="input-label mb-0">{{ t('admin.accounts.openai.responsesWeeklyOverdraft') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
          ...
          <button
            type="button"
            data-testid="openai-responses-weekly-overdraft-toggle"
            role="switch"
            :aria-checked="openAIResponsesWeeklyOverdraftEnabled"
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  if (newAccount.platform === 'openai' && (newAccount.type === 'oauth' || newAccount.type === 'setup-token' || newAccount.type === 'apikey')) {
    ...
    openAIResponsesWeeklyOverdraftEnabled.value = extra?.openai_responses_weekly_overdraft_enabled === true
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.platform === 'openai' && (props.account.type === 'oauth' || props.account.type === 'setup-token' || props.account.type === 'apikey')) {
      ...
      if (isSparkShadow.value) {
        delete newExtra.openai_responses_weekly_overdraft_enabled
      } else if (openAIResponsesWeeklyOverdraftEnabled.value) {
        newExtra.openai_responses_weekly_overdraft_enabled = true
      } else {
        delete newExtra.openai_responses_weekly_overdraft_enabled
      }
