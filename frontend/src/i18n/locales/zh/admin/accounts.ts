export default {
    accounts: {
      ...
      openai: {
        ...
        responsesWeeklyOverdraft: 'Responses 周配额透支',
        responsesWeeklyOverdraftDesc: '默认关闭。仅当该 OpenAI 账号需要为 /responses 路由注入周配额透支行为时开启。',
        ...
        codexFingerprintModeDesc: 'OpenAI OAuth 的账号级身份策略。默认单会话随机：四维分域且账号内稳定，适合一次运行一个任务；同时运行多个窗口或多人共享时请选择多会话并发，保留 installation/session 稳定并按客户端会话隔离 thread/window。只有显式关闭才透传客户端标识。',
        codexFingerprintOff: '关闭（透传）',
        codexFingerprintDevice: '仅设备',
        codexFingerprintSession: '设备+会话',
        codexFingerprintFull: '完全收敛',
        codexFingerprintRandom: '独立随机（单会话）',
        codexFingerprintRandomMulti: '独立随机（多会话并发）',
